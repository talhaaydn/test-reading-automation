...
@section('title', 'Ders Kazanım Ekleme Ekranı')

@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">{{ $course->name }} Dersi Kazanım Ekleme Ekranı</h3>
        </div>
        
        <form class="form-horizontal" method="POST" action="{{ route('gain.store') }}">
            @csrf
                ...
                @if ($gainCount > 0)
                    @foreach ($gains as $index => $item)                        
                        <div class="form-group" id="inserted_qualification">
                            <div class="row">
                                <label for="name" class="col-sm-2 control-label" style="line-height: 30px;">Kazanım - {{ ++$index }}</label>                                
                                <div class="col-sm-9">
                                    <input 
                                        type="text" 
                                        class="form-control"
